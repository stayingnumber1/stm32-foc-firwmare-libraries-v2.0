...
#if defined HALL_SENSORS//霍尔传感器 
#include "stm32f10x_hall.h"
#define GET_ELECTRICAL_ANGLE    HALL_GetElectricalAngle()//霍尔位置电角度
#define GET_SPEED_0_1HZ         HALL_GetSpeed()
#define GET_SPEED_DPP           HALL_GetRotorFreq()//电机转速
#endif

#if defined ENCODER//编码器
#include "stm32f10x_encoder.h"
#define GET_ELECTRICAL_ANGLE    ENC_Get_Electrical_Angle()//编码器位置角度
#define GET_SPEED_0_1HZ         ENC_Get_Mechanical_Speed()
#define GET_SPEED_DPP    (s16)((ENC_Get_Mechanical_Speed()*_0_1HZ_2_128DPP)/128)//编码器输出的转速
#endif

#if defined NO_SPEED_SENSORS//没有速度传感器
...
#if defined VIEW_HALL_FEEDBACK//霍尔反馈
#include "stm32f10x_hall.h"
#endif

#if defined VIEW_ENCODER_FEEDBACK//编码器反馈
#include "stm32f10x_encoder.h"
#endif

#ifdef ICS_SENSORS//电流传感器反馈
